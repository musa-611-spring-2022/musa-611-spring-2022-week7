var map = L.map('map').setView([40.671247425857224, -73.8100892819502], 11);
...
var queensSites = quality.filter(function (el) {
  return el.properties.District == "Queens"
});

var queensBoundary = BoroughBoundaries.filter(function (el) {
  return el.properties.boro_name == "Queens"
});
...
let filteredSites = () => {
  ...
  if (dateVal !== '' && siteVal === '') {
    return queensSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal);
  }
  if (dateVal === '' && siteVal !== '') {
    return queensSites.filter(site => site.properties['Sample.Site'] === siteVal);
  }
  return queensSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal && site.properties['Sample.Site'] === siteVal);
};
...
L.geoJson(queensBoundary, { style: districtStyle }).addTo(map);
initializeSiteChoices();
updatesiteMarkers(queensSites);
updatechart(queensSites)
