var map = L.map('map').setView([40.78296098548444, -73.96374001994559], 11);
...
var manhattanSites = quality.filter(function (el) {
  return el.properties.District == "Manhattan"
});


var manhattanBoundary = BoroughBoundaries.filter(function (el) {
  return el.properties.boro_name == "Manhattan"
});
...
let filteredSites = () => {
  ...
  if (dateVal === '' && siteVal === '') {
    return manhattanSites;
  }
  if (dateVal !== '' && siteVal === '') {
    return manhattanSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal);
  }
  if (dateVal === '' && siteVal !== '') {
    return manhattanSites.filter(site => site.properties['Sample.Site'] === siteVal);
  }
  return manhattanSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal && site.properties['Sample.Site'] === siteVal);
};
...
L.geoJson(manhattanBoundary, { style: districtStyle }).addTo(map);
initializeSiteChoices();
updatesiteMarkers(manhattanSites);
updatechart(manhattanSites)
