var map = L.map('map').setView([40.585706983867276, -74.15231385463828], 11);
...
var statenIslandSites = quality.filter(function (el) {
  return el.properties.District == "Staten Island"
});

var statenIslandBoundary = BoroughBoundaries.filter(function (el) {
  return el.properties.boro_name == "Staten Island"
});
...
let initializeSiteChoices = () => {
  let siteArr = [];
  statenIslandSites.forEach(site => {
...
let filteredSites = () => {
  ...
  if (dateVal === '' && siteVal === '') {
    return statenIslandSites;
  }
  if (dateVal !== '' && siteVal === '') {
    return statenIslandSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal);
  }
  if (dateVal === '' && siteVal !== '') {
    return statenIslandSites.filter(site => site.properties['Sample.Site'] === siteVal);
  }
  return statenIslandSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal && site.properties['Sample.Site'] === siteVal);
};
...
L.geoJson(statenIslandBoundary, { style: districtStyle }).addTo(map);
initializeSiteChoices();
updatesiteMarkers(statenIslandSites);
updatechart(statenIslandSites)
