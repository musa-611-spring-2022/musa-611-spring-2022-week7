var map = L.map('map').setView([40.854755041969, -73.87595510177002], 12);
...
var bronxSites = quality.filter(function (el) {
  return el.properties.District == "Bronx"
});

var bronxBoundary = BoroughBoundaries.filter(function (el) {
  return el.properties.boro_name == "Bronx"
});
...
let filteredSites = () => {
  ...
  if (dateVal !== '' && siteVal === '') {
    return bronxSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal);
  }
  if (dateVal === '' && siteVal !== '') {
    return bronxSites.filter(site => site.properties['Sample.Site'] === siteVal);
  }
  return bronxSites.filter(site => site.properties['Sample.Date'].slice(0, 4) === dateVal && site.properties['Sample.Site'] === siteVal);
};
...
L.geoJson(bronxBoundary, { style: districtStyle }).addTo(map);
initializeSiteChoices();
updatesiteMarkers(bronxSites);
updatechart(bronxSites)
